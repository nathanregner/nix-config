{ pkgs, lib, ... }: {
  ...
  services.mediamtx = {
    ...
    settings = {
      paths.cam = {
        runOnInit = let ffmpeg = pkgs.unstable.ffmpeg-headless;
        in "${
          lib.getExe ffmpeg
        } -f v4l2 -i /dev/video0 -pix_fmt yuv420p -s:v 1920x1080 -r 25 -c:v libx264 -bf 0 -f rtsp rtsp://localhost:$RTSP_PORT/$MTX_PATH";
        runOnInitRestart = true;
      };
      # https://github.com/bluenviron/mediamtx#webrtc-specific-features
      webrtcAdditionalHosts = [ "voron" "voron.nregner.net" ];
    };
  };
  ...
  networking.firewall = let ports = [ 80 8189 8889 ];
  in {
