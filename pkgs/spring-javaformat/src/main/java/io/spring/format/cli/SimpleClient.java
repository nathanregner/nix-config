...
public class SimpleClient implements Runnable {
    ...
    public void run() {
        ...
        try {
            while (!this.stopping) {
                ...
                var handler = new Handler(socket, this);
        ...
        var openhandlers = this.handlers.elements();
        while (openhandlers.hasMoreElements()) {
            var handler = openhandlers.nextElement();
    ...
    public static class Handler implements Runnable {
        ...
        private final SimpleClient parent;
        ...
        public Handler(Socket s, SimpleClient p) {
            super();
            this.socket = s;
            this.parent = p;
        }
            ...
            while (true) {
                try {
                    ...
                    switch (x) {
                        ...
                        // Create a file
                        case '2':
                            var name = this.getDirectoryName() + "/SimpleApplication." + this.getConnectionNumber() + ".tmp";
                            try {
                        ...
                        case '5':
                            var name1 = this.getDirectoryName() + "/a/b/c/d/e" + "/SimpleApplication." + this.getConnectionNumber() + ".tmp";
                            try {
